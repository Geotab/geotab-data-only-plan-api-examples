...
{
    static class GetFeedIoxAddOnAsyncExample
    {
        public static async Task Run(GeotabDataOnlyPlanAPI api)
        {
            ConsoleUtility.LogExampleStarted(typeof(GetFeedIoxAddOnAsyncExample).Name);
            ...
            try
            {
                ...
                long? feedVersion = 0;
                FeedResult<IoxAddOn> feedResult;

                // Make initial GetFeed call.  The returned toVersion will be used as the fromVersion to start the subsequent GetFeed loop.
                feedResult = await api.GetFeedIoxAddOnAsync(feedVersion, getFeedresultsLimit);
            ...
            ConsoleUtility.LogExampleFinished(typeof(GetFeedIoxAddOnAsyncExample).Name);
        }
    }
}